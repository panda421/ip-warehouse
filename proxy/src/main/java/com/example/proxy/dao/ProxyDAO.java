...
public class ProxyDAO {
    ...
    private static final Pattern proxyPattern = Pattern.compile(PROXY_RULE);
     ...
     * @return
     */
    public CommonResult save(String proxy) {
        if (proxyPattern.matcher(proxy).matches()) {
            if (!exists(proxy)) {
                ...
                return CommonResult.error(CodeEnum.PROXY_EXISTS);
            }
        }
        return CommonResult.error(CodeEnum.PARAMS_ERROR);
    }
